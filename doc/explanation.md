...
This document discusses why `@seneca/meetup-provider` is built the way it is.
...
The payoff is uniformity. An application reading from meetup, a
payment processor and a CRM uses one access pattern for all three:

```js
await seneca.entity('provider/meetup/event').list$()
...
`provider/meetup/` canon, registering the `role:entity` messages
...
*already* entity-shaped. The meetup SDK exposes accessors like
`client.Event()` — carrying
...
The API nests `event` under a parent resource: a `event`'s URL contains `group_urlname`.
...
`group_urlname` is required on every `event` command, and why
`event` `load$` takes an object rather than a bare id string.
This is inherited from the API's URL structure — `events` — rather than
chosen here.

The provider checks for `group_urlname` itself and throws a named error
...
indistinguishable from "that event does not exist" — which the provider
...
`event`, and the store surface those
...
`meetup` and sends the `apikey` as a bearer token when one is
configured.
...
The SDK is an ordinary published dependency: `@voxgig-sdk/meetup` at
...
[@voxgig/sdkgen](https://github.com/voxgig/sdkgen) from the meetup API
...
The source of truth is the SDK project's model — `../../voxgig-sdk/meetup-sdk` from
